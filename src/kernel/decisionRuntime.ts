 ...
 *
 * hold 上限：同一 session 累计 hold 次数 >= HOLD_LIMIT → 自动升级为 reject
 */
export async function runDecisionRuntime(input: RuntimeInput): Promise<RuntimeResult> {
  const { intentId, sessionId, packId, kernel, confidence } = input;

  // Query hold count across entire session (joins intents → decisions)
  ...
  const [decision] = await query<LiosDecision>(
    `INSERT INTO lios_decisions
       (intent_id, pack_id, decision_type, rationale, confidence, hold_count, metadata)
